...
│   ├── views/          ← 各模块页面（按需在子目录中创建；_draft/ 子目录为双速轨草稿区，未晋升不得进交接）
│   ├── mock/           ← 模块 Mock 与共享枚举
│   ├── router/         ← 路由配置
...
| `/项目与模块启动` | 项目级生成 P07；模块级先选档：文档先行（PRD → SRS → 流程图 → 原型）或双速轨（先出草稿原型给客户看，方向确认后再反向定稿文档）；翻新走 §2.3-翻新 |
